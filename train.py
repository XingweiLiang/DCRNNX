...
def main(args):
    ...
    if args.eval:
        s = ModelTrainer(**vars(args))
        print("Model %s loaded from previous state!" % args.initial_model)
        s.load_parameters(args.initial_model)
        uar, true_list, pred_list = s.eval_network(args.devel_path, args.wav_path, args.feature_type, feat_df,
                                                   args.mel, args.n_mel)
        cm_analysis(true_list, pred_list, args.save_path + '/{args.feature_type}', sorted(set(true_list)))
        df = pd.DataFrame({'true': true_list, 'pred': pred_list})
        # df.to_excel(f'results/{args.feature_type}.xlsx', index=False)
        df.to_excel('results/cnn.xlsx', index=False)
        # df.to_csv(f'results/test.csv', sep=',', index=False)
        print("UAR %2.2f" % uar)
        return
    ...
    score_file.write('Model: {args.model} \n'
                     'Feature Type: {args.feature_type} \n'
                     'Spectrogram : {args.mel}, n_mel: {args.n_mel} \n'
                     + '-' * 20 + '\n')

    # 模型训练
    for epoch in range(1, args.epoch + 1):
        print("-" * 10, '第{epoch}轮训练开始', "-" * 10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wav_path', type=str, default='dist/wav', help='Path of the WAV file')
    ...
    parser.add_argument('--save_path', type=str, default="exps/exp-cnn_new",
                        help='Path to save the score.txt and models')
    parser.add_argument('--eval', type=bool, default=False, help='eval on-off')  #train or eval
    parser.add_argument('--initial_model', type=str, default='exps/exp-cnn_new/model/model_0016.model',
                        help='Path of the initial_model file') #'exps/exp-cnn/model/model_0007.model'
    ...
    parser.add_argument('--n_mel', type=str, default=80, help='nmel')
    parser.add_argument('--model', type=str, default='hwtransformer', help='Learning rate')  #这里可以修改model   hwmodel  hwtransformer  twochannel
    parser.add_argument('--lr', type=float, default=0.001, help='Learning rate')
    parser.add_argument("--lr_decay", type=float, default=0.97, help='Learning rate decay every [test_step] epochs')  #0.97
    ...
    main(args)
    print('模型训练结束，共耗时：{round(time.time() - st, 2)}s')
