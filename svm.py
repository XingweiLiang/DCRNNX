...
def run_svm(feature_folder, label_base, result_folder, metrics_folder):
    ...
    df_predictions_devel = pd.DataFrame(
        {"filename": devel_names, "prediction": devel_preds.tolist(), "true": devel_y.tolist()})
    ...
    df_predictions_test = pd.DataFrame(
        {"filename": test_names, "prediction": test_preds.tolist(), "true": test_y.tolist()})
