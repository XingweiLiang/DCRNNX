...
class TwoChannel(nn.Module):
    ...
    def __init__(self, feature_type, mel, n_mel):
        ...
        self.mel = mel
        self.feature_dim = 126 if feature_type else 64  # 获取指定特征的维度
        ...
        # fc + softmax
        # if feature_type and (self.mel is 'fbank'):
        #     in_features = 192
        in_features = 64
    ...
    def forward(self, x1, x2, aug=True):
        ...
        if self.mel is 'fbank':
            with torch.no_grad():
                x3 = x1
                x3 = self.torchfbank(x3) + 1e-6
                x3 = x3.log()
                x3 = x3 - torch.mean(x3, dim=-1, keepdim=True)
                x3 = x3.unsqueeze(1)  # [32, 1, 80, 103]
                x3 = self.conv1(x3)  # [32, 16, 40, 51]
                x3 = self.conv2(x3)  # [32, 32, 20, 25]
                x3 = x3.reshape(x3.shape[0], -1, x3.shape[-1]).permute(2, 0, 1)  # [25, 32, 32 * 20]
                output, (hc, nc) = self.lstm(x3)  # [25, 32, 128], [2, 32, 128]

                hc = hc.permute(1, 0, 2)  # [32, 2, 128]
                hc = hc.contiguous().view(hc.shape[0], -1)  # [32, 256]
                x3 = self.fc(hc)  # [32, 64]

        x1 = torch.FloatTensor(librosa.feature.mfcc(y=np.array(x1), sr=44100, n_mfcc=self.n_mel))
        ...
        hc = hc.contiguous().view(hc.shape[0], -1)  # [32, 256]
        x1 = self.fc(hc)  # [32, 64]
        y1 = self.classification(x1)  # (32, 6)
        if self.feature_type:
            ...
            x2 = x2.squeeze(-1)
            x2 = self.fc3(x2)
            y2 = self.classification(x2)
            # if self.feature_type and (self.mel is 'fbank'):
            #     x = torch.cat((x1, x2, x3), 1)
            # elif self.feature_type:
            #     x = torch.cat((x1, x2), 1)
            # elif self.mel is 'fbank':
            #     x = torch.cat((x1, x3), 1)
            # else:
            #     x = x1
            ...
            y = np.zeros(shape=(32, 6), dtype=float)

            for i in range(32):
                for j in range(6):
                    y[i][j] = (y1[i][j] + y2[i][j]) / 2
...
if __name__ == '__main__':
    x1 = torch.randn(32, 44100 + 882)
    # x1 = torch.randn(32, 40, 88)
    x2 = torch.randn(32, 126)
