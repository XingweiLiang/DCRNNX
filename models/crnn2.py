...
class CRNN2(nn.Module):
    def __init__(self, feature_type, mel, n_mels=40):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(kernel_size=(3, 3), in_channels=1, out_channels=64, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2),
        )
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(kernel_size=(3, 3), in_channels=64, out_channels=32, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2),
        )

        self.lstm = nn.LSTM(32 * n_mels // 2 // 2,
                            num_layers=2,
                            hidden_size=128,
                            dropout=0.25
                            )
